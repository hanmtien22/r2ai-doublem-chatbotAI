...
class TestIntegratedFallbacks:
    def test_fuzzy_company_resolution_is_used_by_pipeline(self):
        pipeline = QueryPipeline(reference_year=2025, use_llm_fallback=False)
        result = pipeline.process("Doanh thu cua Hoa Phaat nam 2025")
        assert "HPG" in result.entities.tickers

    def test_llm_router_fallback_is_used_for_ambiguous_query(self):
        class FakeLLMClient:
        ...
        result = pipeline.process("Doanh thu nam 2025")
        assert result.query_type == "multi_comparison"
